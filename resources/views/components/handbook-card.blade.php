...
<div class="w-full border rounded-lg overflow-hidden mx-auto p-5">
    <div class="flex flex-wrap flex-col-reverse lg:flex-nowrap lg:flex-row">
        <div class="xl:h-full w-full lg:w-2/5 2xl:w-1/3">
            ...
            <div class="flex flex-wrap xl:flex-nowrap justify-between mt-4">
                @if($category->button_type == 'reservation')
                    <div class="flex items-center justify-center px-2 py-2 mb-2 mr-1 xl:mb-0 w-full bg-lime-600 text-sm text-white text-center font-semibold rounded-lg hover:bg-lime-700 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-400"><a href="{{ route('handbooks.show', $handbook->id) }}">Забронировать</a></div>
                @endif
                @if($category->button_type == 'appointment')
                    <div class="flex items-center justify-center px-2 py-2 mb-2 mr-1 xl:mb-0 w-full bg-lime-600 text-sm text-white text-center font-semibold rounded-lg hover:bg-lime-700 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-400"><a href="{{ route('handbooks.show', $handbook->id) }}">Записаться</a></div>
                @endif
                @if($category->button_type == 'product')
                    <div class="flex items-center justify-center px-2 py-2 mb-2 mr-1 xl:mb-0 w-full bg-lime-600 text-sm text-white text-center font-semibold rounded-lg hover:bg-lime-700 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-400"><a href="{{ route('handbooks.show', $handbook->id) }}">Купить сейчас</a></div>
                @endif
                @if($category->button_type == 'address')
                    <div class="flex items-center justify-center px-2 py-2 mb-2 mr-1 xl:mb-0 w-full bg-lime-600 text-sm text-white text-center font-semibold rounded-lg hover:bg-lime-700 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-400"><a href="{{ route('handbooks.show', $handbook->id) }}">Подробнее</a></div>
                @endif
                <!-- <button href="#" class="flex w-full border-2 disabled:bg-gray-200 disabled:border-gray-500 border-lime-600 px-2 py-2 ml-2.5 text-sm text-lime-600 disabled:text-gray-800 font-semibold rounded-lg hover:bg-lime-700 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-600 justify-center items-center" title="Находится в разработке" disabled>Оставить отзыв</button> -->
            </div>
        ...
        <div class="ml-0 lg:ml-6 w-full lg:w-3/5 2xl:w-2/3">
            @if ($handbook->distance > 999)
                <div class="rounded-md text-sm text-white w-fit font-semibold p-1 bg-lime-600">До объекта {{ intval(round($handbook->distance / 1000)) }} км</div>
            @elseif ($handbook->distance == 0)
            
            @else
                <div class="rounded-md text-sm text-white w-fit font-semibold p-1 bg-lime-600">До объекта {{ intval(round($handbook->distance)) }} метров</div>
            @endif
            <h2 class="text-2xl mt-1.5 font-bold text-gray-800"><a href="{{ route('handbooks.show', $handbook->id) }}">{{ $handbook->title }}</a></h2>
            ...
            <p class="text-gray-500 font-medium my-6">{{ Str::limit(strip_tags(str_replace(['•', '&nbsp;'], '', $handbook->description)), 150) }}</p>
            @if ($handbook->price > 0 && $handbook->handbooksCategories->whereIn('category_id', $animals->pluck('id'))->isNotEmpty())
                <p class="max-[768px]:mb-6 text-xl font-bold">Цена: {{ $handbook->price }} ₽</p>
            @elseif (($handbook->price === 0 || $handbook->price === null) && $handbook->handbooksCategories->whereIn('category_id', $animals->pluck('id'))->isNotEmpty())
                <p class="max-[768px]:mb-6 text-xl font-bold">Цена: Бесплатно</p>
            @endif
