...
@section('content')
    <main class="w-full" id="content" role="main">
        <div class="relative overflow-hidden">
            <div class="max-w-[85rem] mx-auto py-8 px-4 min-[641px]:mt-5">  
                <h2 class="min-[641px]:hidden text-3xl sm:text-4xl lg:text-5xl xl:text-7xl mb-5 md:mt-0 font-bold md:leading-tight w-full">{{ $category->name }}</h2>              
                <div class="flex flex-col md:flex-row md:space-x-8 xl:space-x-16">
                    <div class="max-[640px]:w-full max-[768px]:w-1/3 lg:w-1/4">
                        @if(count(array_filter($handbooks->items(), function($handbook) use ($city) {
                            ...
                            <div class="flex mb-12 max-[640px]:mb-0 border-4 border-lime-200 rounded-3xl bg-map-image bg-no-repeat bg-cover h-[191px] justify-center items-center">
                        @else
                            <div class="flex mb-12 max-[640px]:mb-0 border-4 border-gray-100 rounded-3xl bg-map-image bg-no-repeat bg-cover h-[191px] justify-center items-center">
                        @endif
                                ...
                                <div class="relative flex space-x-3 bg-white justify-center items-center">
                                    <div class="flex w-full">
                                        ...
                                        <input type="text"
                                            name="query"
                                            id="query"
                                            class="py-3 px-4 block w-full text-sm font-medium border border-gray-200 shadow-sm rounded-full focus:z-10 focus:border-lime-500 focus:ring-lime-500"
                                    ...
                                    <button class="flex-[1_0%]" type="submit" hx-boost="true">
                                        <div class="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-full border border-transparent font-semibold bg-lime-400 text-white hover:bg-lime-600 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-500 focus:ring-offset-2 transition-all text-sm">
                                            Поиск
                                        </div>
                                    </button>
                                </div>
                            </form> -->
                            @if ($category->slug == 'zoogostinicy')
                                <div class="mt-11">
                                    <form action="{{ route('categories.show', $category->slug) }}" method="GET">
                                        <p class="text-3xl font-bold" for="title">Выбор отеля</p>
                                        <p class="text-sm font-medium mt-6" for="title">Дата вьезда</p>
                                        <input name="date" class="border-2 rounded-md w-full p-2 mt-3.5" type="date" min="{{ date('Y-m-d') }}">
                                        <p class="text-sm font-medium mt-9" for="title">Дата отъезда</p>
                                        <input name="date" class="border-2 rounded-md w-full p-2 mt-3.5" type="date" min="{{ date('Y-m-d') }}">
                                        <p class="text-sm font-medium mt-9" for="title">Вид животного</p>
                                        <select name="animal" class="border-2 rounded-md w-full p-2 mt-3.5">
                                            <option>Кошка</option>
                                        ...
                                        <p class="text-sm font-medium mt-9" for="title">Цена от и до</p>
                                        <div class="grid grid-cols-2 gap-4 mt-3.5">
                                            <div class="relative mt-2 rounded-md shadow-sm">
                                                <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                                    <span class="text-gray-500 sm:text-sm">₽</span>
                                                </div>
                                                <input type="text" name="price" id="price" class="block w-full rounded-md border-0 py-2 px-7 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm leading-5" placeholder="1080">
                                            </div>
                                            <div class="relative mt-2 rounded-md shadow-sm">
                                                <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                                                    <span class="text-gray-500 sm:text-sm">₽</span>
                                                </div>
                                                <input type="text" name="price" id="price" class="block w-full rounded-md border-0 py-2 px-7 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm leading-5" placeholder="38901">
                                            </div>
                                        </div>
                                        <button class="px-4 py-2 mt-9 w-full bg-lime-600 text-white font-semibold rounded-lg hover:bg-lime-700 hover:transition-all hover:duration-500 focus:outline-none focus:ring-2 focus:ring-lime-700" type="submit">Применить фильтр</button>
                                    </form>
                                </div>
                            @endif
                        </div>
                        @if ($category->slug == 'zoogostinicy')
                            <div class="flex flex-wrap w-full rounded-3xl border-2 border-sky-200 mt-14 p-6 xl:p-5 2xl:p-6 py-8 bg-sky-50">
                                <p class="text-2xl font-bold mb-2.5">Владелец отеля?</p>
                                <p class="text-sm font-medium">Мы готовы бесплатно разместить его нашем портале. Просто нажмите на кнопку ниже.</p>
                                <div class="inline-flex mt-10 cursor-pointer px-4 py-3 border rounded-full border-gray-500 bg-slate-50 hover:text-lime-600 hover:transition-all hover:duration-500">
                                    <span class="mr-2.5 text-sm font-semibold leading-normal">Зарегистрировать отель</span>
                                    <img src="{{ asset('images/assets/icons/button-mailing-list.svg') }}" alt="edit">
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="max-[640px]:w-full max-[768px]:w-2/3 lg:w-3/4 space-y-6">
                        <h2 class="max-[640px]:hidden text-3xl sm:text-4xl lg:text-5xl xl:text-7xl mt-5 md:mt-0 font-bold md:leading-tight w-full">{{ $category->name }}</h2>
                        <div class="flex max-[768px]:flex-wrap items-center">
                            <p class="text-2xl font-bold">Сортировать:</p>
                            <form class="flex max-[380px]:flex-wrap w-full ml-11 max-[768px]:ml-0 max-[768px]:mt-4" id="sortByDistanceForm" action="{{ route('categories.distance', $category->slug) }}" method="GET">
                                @csrf
                                ...
                                <script>
                                    ...
                                    function showPosition(position) {
                                        ...
                                        document.getElementById("sortByDistanceForm").submit();
                                    }
                                </script>
                                @if (isset($handbook) && $handbook->categories->whereIn('id', $animals->pluck('id'))->isNotEmpty())
                                    <a class="flex w-1/2 max-[380px]:w-full rounded-l-xl max-[380px]:rounded-t-xl max-[380px]:rounded-b-none border text-base font-medium py-3 px-4 hover:bg-lime-200 hover:transition-all hover:duration-500" href="{{ route('categories.price', $category->slug) }}">
                                        <img class="mr-2" src="{{ asset('images/assets/icons/coin.svg') }}" alt="">По цене
                                    </a>
                                    <button type="button" class="flex w-1/2 max-[380px]:w-full rounded-r-xl max-[380px]:rounded-t-none max-[380px]:rounded-b-xl border-y border-r max-[380px]:border-x max-[380px]:border-b max-[380px]:border-t-0 text-base font-medium py-3 px-4 hover:bg-lime-200 hover:transition-all hover:duration-500" onclick="getUserLocation()">
                                        <img class="mr-2" src="{{ asset('images/assets/icons/map.svg') }}" alt="">По расстоянию
                                    </button>
                                @else
                                    <button type="button" class="flex w-1/2 max-[380px]:w-full rounded-l-xl max-[380px]:rounded-t-xl max-[380px]:rounded-b-none border text-base font-medium py-3 px-4 hover:bg-lime-200 hover:transition-all hover:duration-500" onclick="getUserLocation()">
                                        <img class="mr-2" src="{{ asset('images/assets/icons/map.svg') }}" alt="">По расстоянию
                                    </button>
                                    <a class="flex w-1/2 max-[380px]:w-full rounded-r-xl max-[380px]:rounded-t-none max-[380px]:rounded-b-xl border-y border-r max-[380px]:border-x max-[380px]:border-b max-[380px]:border-t-0 text-base font-medium py-3 px-4 hover:bg-lime-200 hover:transition-all hover:duration-500" href="{{ route('categories.rating', $category->slug) }}">
                        ...
                        @foreach($handbooks as $handbook)
                            @if(str_contains($handbook->address, $city->name))
                                ...
                                @endphp
                                @component('components.handbook-card', ['handbook' => $handbook, 'category' => $category, 'animals' => $animals, 'categories' => $categories])
                                @endcomponent
                            @endif
                            {{ $handbook->distance_column }}
                        @endforeach
                                ...
                                <ul class="grid grid-rows-1 grid-flow-col gap-2 pl-0 rounded list-none flex-wrap">
                                    ...
                                    @else
                                        <li class="disabled" aria-disabled="true" aria-label="@lang('pagination.next')">
                                            <span class="text-sm rounded-lg font-bold px-4 py-2 leading-relaxed border border-gray-300 text-gray-500 cursor-not-allowed hover:bg-gray-300 transition duration-300 ease-in-out" aria-hidden="true">&rsaquo;</span>
                                        </li>
                                    @endif
                                </ul>
                            </nav>
